...
void WorldSession::HandleIgnoreTradeOpcode(
    WorldPackets::Trade::IgnoreTrade & /*ignoreTrade*/) {}

void WorldSession::HandleBusyTradeOpcode(
    WorldPackets::Trade::BusyTrade & /*busyTRade*/) {}

void WorldSession::SendUpdateTrade(bool trader_data /*= true*/) {
  TradeData *view_trade = trader_data ? _player->GetTradeData()->GetTraderData()
  ...
  for (uint8 i = 0; i < TRADE_SLOT_COUNT; ++i) {
    if (Item const *item = view_trade->GetItem(TradeSlots(i))) {
      ...
      if (!item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_WRAPPED)) {
        WorldPackets::Trade::UnwrappedTradeItem &unwrappedItem =
        ...
        unwrappedItem.ReforgeEnchantmentID =
        ...
        unwrappedItem.Lock =
            item->GetTemplate()->GetLockID() &&
            !item->HasFlag(ITEM_FIELD_FLAGS, ITEM_FIELD_FLAG_UNLOCKED);
        unwrappedItem.MaxDurability =
        ...
        for (uint32 s = SOCK_ENCHANTMENT_SLOT; s < MAX_GEM_SOCKETS; ++s)
          unwrappedItem.SocketEnchant[s] = item->GetEnchantmentId(
              EnchantmentSlot(s + SOCK_ENCHANTMENT_SLOT));
      }
...
void WorldSession::moveItems(Item *myItems[], Item *hisItems[]) {
  ...
  for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i) {
    ...
    bool traderCanTrade =
        (myItems[i] == nullptr ||
         trader->CanStoreItem(NULL_BAG, NULL_SLOT, traderDst, myItems[i],
                              false) == EQUIP_ERR_OK);
    bool playerCanTrade =
        (hisItems[i] == nullptr ||
         _player->CanStoreItem(NULL_BAG, NULL_SLOT, playerDst, hisItems[i],
                               false) == EQUIP_ERR_OK);
    if (traderCanTrade && playerCanTrade) {
      // Ok, if trade item exists and can be stored
      // If we trade in both directions we had to check, if the trade will work
      // before we actually do it A roll back is not possible after we stored it
      if (myItems[i]) {
        // logging
        LOG_DEBUG("network", "partner storing: %u",
                     myItems[i]->GetGUID().GetCounter());
        if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE)) {
          sLog->outCommand(
              _player->GetSession()->GetAccountId(),
              "GM %s (Account: %u) trade: %s (Entry: %d Count: %u) to player: "
              "%s (Account: %u)",
              _player->GetName().c_str(), _player->GetSession()->GetAccountId(),
              myItems[i]->GetTemplate()->GetDefaultLocaleName(),
          ...
          myItems[i]->SetUInt32Value(
              ...
              trader->GetTotalPlayedTime() -
                  (_player->GetTotalPlayedTime() -
                   myItems[i]->GetUInt32Value(ITEM_FIELD_CREATE_PLAYED_TIME)));
        // store
        trader->MoveItemToInventory(traderDst, myItems[i], true, true);
      }
      if (hisItems[i]) {
        // logging
        LOG_DEBUG("network", "player storing: %u",
                     hisItems[i]->GetGUID().GetCounter());
        if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE)) {
          sLog->outCommand(trader->GetSession()->GetAccountId(),
                           "GM %s (Account: %u) trade: %s (Entry: %d Count: "
                           "%u) to player: %s (Account: %u)",
                           trader->GetName().c_str(),
                           trader->GetSession()->GetAccountId(),
                           hisItems[i]->GetTemplate()->GetDefaultLocaleName(),
        ...
        if (hisItems[i]->HasFlag(ITEM_FIELD_FLAGS,
          ...
          hisItems[i]->SetUInt32Value(
              ...
              _player->GetTotalPlayedTime() -
                  (trader->GetTotalPlayedTime() -
                   hisItems[i]->GetUInt32Value(ITEM_FIELD_CREATE_PLAYED_TIME)));
        // store
      ...
      if (myItems[i]) {
        if (!traderCanTrade)
          LOG_ERROR("network", "trader can't store item: %u",
                       myItems[i]->GetGUID().GetCounter());
        if (_player->CanStoreItem(NULL_BAG, NULL_SLOT, playerDst, myItems[i],
                                  false) == EQUIP_ERR_OK)
          _player->MoveItemToInventory(playerDst, myItems[i], true, true);
        else
          LOG_ERROR("network", "player can't take item back: %u",
                       myItems[i]->GetGUID().GetCounter());
      }
      // return the already removed items to the original owner
      if (hisItems[i]) {
        if (!playerCanTrade)
          LOG_ERROR("network", "player can't store item: %u",
                       hisItems[i]->GetGUID().GetCounter());
        if (trader->CanStoreItem(NULL_BAG, NULL_SLOT, traderDst, hisItems[i],
                                 false) == EQUIP_ERR_OK)
          trader->MoveItemToInventory(traderDst, hisItems[i], true, true);
        else
          LOG_ERROR("network", "trader can't take item back: %u",
                       hisItems[i]->GetGUID().GetCounter());
      }
    }
  }
...
static void setAcceptTradeMode(TradeData *myTrade, TradeData *hisTrade,
  ...
  for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i) {
    if (Item *item = myTrade->GetItem(TradeSlots(i))) {
      LOG_DEBUG("network", "player trade item %u bag: %u slot: %u",
                   item->GetGUID().GetCounter(), item->GetBagSlot(),
                   item->GetSlot());
      // Can return nullptr
      myItems[i] = item;
    ...
    if (Item *item = hisTrade->GetItem(TradeSlots(i))) {
      LOG_DEBUG("network", "partner trade item %u bag: %u slot: %u",
                   item->GetGUID().GetCounter(), item->GetBagSlot(),
                   item->GetSlot());
      hisItems[i] = item;
...
void WorldSession::HandleAcceptTradeOpcode(
    WorldPackets::Trade::AcceptTrade &acceptTrade) {
  ...
  Item *myItems[TRADE_SLOT_TRADED_COUNT] = {nullptr, nullptr, nullptr,
                                            nullptr, nullptr, nullptr};
  Item *hisItems[TRADE_SLOT_TRADED_COUNT] = {nullptr, nullptr, nullptr,
                                             nullptr, nullptr, nullptr};
  ...
  for (uint8 i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i) {
    ...
    if (Item *item = his_trade->GetItem(TradeSlots(i))) {
      ...
      // if (item->IsBindedNotWith(_player))   // dont mark as invalid when his
      // item isnt good (not exploitable because if item is invalid trade will
      // fail anyway later on the same check)
      //{
  ...
  if (his_trade->IsAccepted()) {
    ...
    hisCanCompleteInfo.BagResult = trader->CanStoreItems(
        myItems, TRADE_SLOT_TRADED_COUNT, &hisCanCompleteInfo.ItemID);
    myCanCompleteInfo.BagResult = _player->CanStoreItems(
        hisItems, TRADE_SLOT_TRADED_COUNT, &myCanCompleteInfo.ItemID);
    ...
    if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE)) {
      if (my_trade->GetMoney() > 0) {
        sLog->outCommand(_player->GetSession()->GetAccountId(),
                         "GM %s (Account: %u) give money (Amount: " UI64FMTD
                         ") to player: %s (Account: %u)",
                         _player->GetName().c_str(),
                         _player->GetSession()->GetAccountId(),
                         my_trade->GetMoney(), trader->GetName().c_str(),
                         trader->GetSession()->GetAccountId());
      }
      ...
      if (his_trade->GetMoney() > 0) {
        sLog->outCommand(trader->GetSession()->GetAccountId(),
                         ...
                         trader->GetName().c_str(),
                         trader->GetSession()->GetAccountId(),
                         his_trade->GetMoney(), _player->GetName().c_str(),
                         _player->GetSession()->GetAccountId());
      }
    }
    ...
    // desynchronized with the other saves here (SaveInventoryAndGoldToDB() not
    // have own transaction guards)
...
void WorldSession::HandleUnacceptTradeOpcode(
    WorldPackets::Trade::UnacceptTrade & /*unacceptTrade*/) {
...
void WorldSession::HandleBeginTradeOpcode(
    WorldPackets::Trade::BeginTrade & /*beginTrade*/) {
  TradeData *my_trade = _player->m_trade;
  if (!my_trade)
    return;
...
void WorldSession::HandleCancelTradeOpcode(
    WorldPackets::Trade::CancelTrade & /*cancelTrade*/) {
...
void WorldSession::HandleInitiateTradeOpcode(
    WorldPackets::Trade::InitiateTrade &initiateTrade) {
  ...
  if (GetPlayer()->getLevel() < sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ)) {
    SendNotification(GetFirelandsString(LANG_TRADE_REQ),
  ...
  if (pOther->GetTeam() != _player->GetTeam() &&
      (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_TRADE) &&
       !GetPlayer()->GetSession()->HasPermission(
           rbac::RBAC_PERM_ALLOW_TWO_SIDE_TRADE))) {
  ...
  if (pOther->getLevel() < sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ)) {
    SendNotification(GetFirelandsString(LANG_TRADE_OTHER_REQ),
                     sWorld->getIntConfig(CONFIG_TRADE_LEVEL_REQ));
    return;
  }
...
void WorldSession::HandleSetTradeGoldOpcode(
    WorldPackets::Trade::SetTradeGold &setTradeGold) {
  TradeData *my_trade = _player->GetTradeData();
  if (!my_trade)
    return;
...
void WorldSession::HandleSetTradeItemOpcode(
    WorldPackets::Trade::SetTradeItem &setTradeItem) {
  TradeData *my_trade = _player->GetTradeData();
  if (!my_trade)
    return;
  ...
  Item *item =
      _player->GetItemByPos(setTradeItem.PackSlot, setTradeItem.ItemSlotInPack);
  if (!item || (setTradeItem.TradeSlot != TRADE_SLOT_NONTRADED &&
  ...
  if (setTradeItem.TradeSlot != TRADE_SLOT_NONTRADED &&
...
void WorldSession::HandleClearTradeItemOpcode(
    WorldPackets::Trade::ClearTradeItem &clearTradeItem) {
  TradeData *my_trade = _player->m_trade;
  if (!my_trade)
    return;
