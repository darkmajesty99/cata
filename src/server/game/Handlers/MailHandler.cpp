...
bool WorldSession::CanOpenMailBox(ObjectGuid guid) {
  if (guid == _player->GetGUID()) {
    if (!HasPermission(rbac::RBAC_PERM_COMMAND_MAILBOX)) {
      LOG_WARN("cheat", "%s attempted to open mailbox by using a cheat.",
...
void WorldSession::HandleSendMail(WorldPacket &recvData) {
  ...
  if (player->getLevel() < sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ)) {
    SendNotification(GetFirelandsString(LANG_MAIL_SENDER_REQ),
  ...
  if (!receiverGuid) {
    LOG_INFO(
        "network",
        "Player %u is sending mail to %s (GUID: non-existing!) with subject %s "
        "and body %s includes %u items, " UI64FMTD " copper and " UI64FMTD
        " COD copper with unk1 = %u, unk2 = %u",
        player->GetGUID().GetCounter(), receiverName.c_str(), subject.c_str(),
  ...
  LOG_INFO(
      "network",
      "Player %u is sending mail to %s (%s) with subject %s and body %s "
      "including %u items, " UI64FMTD " copper and " UI64FMTD
      " COD copper with unk1 = %u, unk2 = %u",
      player->GetGUID().GetCounter(), receiverName.c_str(),
      receiverGuid.ToString().c_str(), subject.c_str(), body.c_str(),
  ...
  uint8 mailsCount = 0; // do not allow to send to one player more than 100
                        // mails
  ...
  if (receiver) {
    ...
    if (CharacterCacheEntry const *characterInfo =
    ...
    CharacterDatabasePreparedStatement *stmt =
        CharacterDatabase.GetPreparedStatement(CHAR_SEL_MAIL_COUNT);
  ...
  for (uint8 i = 0; i < items_count; ++i) {
    if (Item *item = player->GetItemByGuid(itemGUIDs[i])) {
      ...
      if (!itemProto ||
          !(itemProto->GetFlags() & ITEM_FLAG_IS_BOUND_TO_ACCOUNT)) {
        accountBound = false;
        break;
      }
    }
  ...
  if (receiverLevel < sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ)) {
    SendNotification(GetFirelandsString(LANG_MAIL_RECEIVER_REQ),
                     sWorld->getIntConfig(CONFIG_MAIL_LEVEL_REQ));
    return;
  }
  ...
  for (uint8 i = 0; i < items_count; ++i) {
    ...
    // prevent sending bag with items (cheat: can be placed in bag after adding
    // equipped empty bag to mail)
    if (!item) {
      player->SendMailResult(0, MAIL_SEND, MAIL_ERR_MAIL_ATTACHMENT_INVALID);
      return;
    }
    ...
    if (item->GetTemplate()->GetFlags() & ITEM_FLAG_CONJURED ||
  ...
  if (items_count > 0 || money > 0) {
    ...
    if (items_count > 0) {
      for (uint8 i = 0; i < items_count; ++i) {
        ...
        if (log) {
          sLog->outCommand(
              GetAccountId(),
              "GM %s (GUID: %u) (Account: %u) mail item: %s (Entry: %u Count: "
              "%u) "
              "to: %s (%s) (Account: %u)",
              GetPlayerName().c_str(), GetGUIDLow(), GetAccountId(),
              item->GetTemplate()->GetDefaultLocaleName(), item->GetEntry(),
              item->GetCount(), receiverName.c_str(),
              receiverGuid.ToString().c_str(), receiverAccountId);
        }

        item->SetNotRefundable(
            GetPlayer()); // makes the item no longer refundable
        player->MoveItemFromInventory(items[i]->GetBagSlot(), item->GetSlot(),
                                      true);
    ...
    if (log && money > 0) {
      sLog->outCommand(GetAccountId(),
                       "GM %s (GUID: %u) (Account: %u) mail money: " UI64FMTD
                       " to: %s (%s) (Account: %u)",
                       GetPlayerName().c_str(), GetGUIDLow(), GetAccountId(),
                       money, receiverName.c_str(),
                       receiverGuid.ToString().c_str(), receiverAccountId);
    }
  }

  // If theres is an item, there is a one hour delivery delay if sent to another
  // account's character.
  uint32 deliver_delay =
      needItemDelay ? sWorld->getIntConfig(CONFIG_MAIL_DELIVERY_DELAY) : 0;

  // Mail sent between guild members arrives instantly if they have the guild
  // perk "Guild Mail"
  ...
  draft.AddMoney(money).AddCOD(COD).SendMailTo(
      trans, MailReceiver(receiver, receiverGuid.GetCounter()),
      MailSender(player),
...
void WorldSession::HandleMailReturnToSender(WorldPacket &recvData) {
  ...
  if (!CanOpenMailBox(
          _player->PlayerTalkClass->GetInteractionData().SourceGuid))
    return;
  ...
  if (!m || m->state == MAIL_STATE_DELETED ||
    ...
    player->SendMailResult(mailId, MAIL_RETURNED_TO_SENDER,
  ...
  if (m->messageType == MAIL_NORMAL && m->sender) {
    ...
    draft.AddMoney(m->money).SendReturnToSender(GetAccountId(), m->receiver,
                                                m->sender, trans);
  }
...
void WorldSession::HandleMailTakeItem(WorldPacket &recvData) {
  ...
  // without paying
  if (std::find_if(m->items.begin(), m->items.end(),
                   [itemId](MailItemInfo info) {
  ...
  if (msg == EQUIP_ERR_OK) {
    ...
    if (m->COD > 0) // if there is COD, take COD money from player and send them
                    // to sender by mail
    {
      ...
      if (HasPermission(rbac::RBAC_PERM_LOG_GM_TRADE)) {
        ...
        if (receiver) {
          ...
          if (!sCharacterCache->GetCharacterNameByGuid(sender_guid,
                                                       sender_name))
            sender_name =
                sObjectMgr->GetFirelandsStringForDBCLocale(LANG_UNKNOWN);
        }
        sLog->outCommand(
            GetAccountId(),
            "GM %s (Account: %u) receiver mail item: %s (Entry: %u Count: %u) "
            "and send COD money: " UI64FMTD " to player: %s (Account: %u)",
            GetPlayerName().c_str(), GetAccountId(),
            it->GetTemplate()->GetDefaultLocaleName(), it->GetEntry(),
            it->GetCount(), m->COD, sender_name.c_str(), sender_accId);
      } else if (!receiver)
        sender_accId =
            sCharacterCache->GetCharacterAccountIdByGuid(sender_guid);
      ...
      if (receiver || sender_accId) {
        MailDraft(m->subject, "")
            .AddMoney(m->COD)
            .SendMailTo(trans, MailReceiver(receiver, m->sender),
...
void WorldSession::HandleGetMailList(WorldPacket &recvData) {
  ...
  for (PlayerMails::iterator itr = player->GetMailBegin();
       itr != player->GetMailEnd(); ++itr) {
    ...
    size_t next_mail_size =
        2 + 4 + 1 + ((*itr)->messageType == MAIL_NORMAL ? 8 : 4) + 4 * 8 +
        ((*itr)->subject.size() + 1) + ((*itr)->body.size() + 1) + 1 +
        item_count * (1 + 4 + 4 + MAX_INSPECTED_ENCHANTMENT_SLOT * 3 * 4 + 4 +
                      4 + 4 + 4 + 4 + 4 + 1);
    ...
    data << float(float((*itr)->expire_time - GameTime::GetGameTime()) /
                  DAY);                     // Time
    ...
    for (uint8 i = 0; i < item_count; ++i) {
      ...
      for (uint8 j = 0; j < MAX_INSPECTED_ENCHANTMENT_SLOT; ++j) {
        ...
        data << uint32(
            (item ? item->GetEnchantmentDuration((EnchantmentSlot)j) : 0));
        data << uint32(
  ...
  data.put<uint32>(0, realCount); // this will display warning about undelivered
...
void WorldSession::HandleMailCreateTextItem(WorldPacket &recvData) {
  ...
  if (!m || (m->body.empty() && !m->mailTemplateId) ||
    ...
    player->SendMailResult(mailId, MAIL_MADE_PERMANENT,
                           MAIL_ERR_INTERNAL_ERROR);
    return;
  }
  ...
  if (!bodyItem->Create(sObjectMgr->GetGenerator<HighGuid::Item>().Generate(),
                        MAIL_BODY_ITEM_TEMPLATE, player)) {
    delete bodyItem;
    return;
  }
  ...
  if (msg == EQUIP_ERR_OK) {
    ...
    player->SendMailResult(mailId, MAIL_MADE_PERMANENT, MAIL_ERR_EQUIP_ERROR,
                           msg);
    delete bodyItem;
  }
}
...
void WorldSession::HandleQueryNextMailTime(WorldPacket & /*recvData*/) {
  ...
  if (_player->unReadMails > 0) {
    ...
    for (PlayerMails::iterator itr = _player->GetMailBegin();
      ...
      data << uint64(m->messageType == MAIL_NORMAL
                         ? ObjectGuid(HighGuid::Player, m->sender)
                         : ObjectGuid::Empty); // player guid
      data << uint32(m->messageType != MAIL_NORMAL ? m->sender
